    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if event.button.id == "create_bot":
            ...
            try:
                ...
                error_post = Post(content="Error: bots.json is malformed or corrupted.", sender="SYSTEM")
...
class BotSocialApp(App):
    ...
    async def on_input_submitted(self, event: Input.Submitted) -> None:
        if event.input.id == "topic_input":
            topic = event.value
            if topic:
                new_post = await asyncio.to_thread(self.create_post, f"Let's talk about: {topic}", "USER")
                self.query_one(PostView).add_post(new_post)
                event.input.value = ""
    ...
    async def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if event.button.id == "create_bot":
            ...
            await self.load_bots_from_json()
        elif event.button.id == "save_bots":
            await self.save_bots_to_json()
        elif event.button.id == "start_chat":
            self.bot_timer.resume()
        elif event.button.id == "stop_chat":
            self.bot_timer.pause()
